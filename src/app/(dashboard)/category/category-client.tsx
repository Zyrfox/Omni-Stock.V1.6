      ...
      {SATUAN_OPTS.map((u) => (
        <button key={u} type="button" onClick={() => setter(u)}
          style={{ padding: "2px 8px", borderRadius: 10,
            border: `1px solid ${val === u ? "rgba(200,241,53,0.5)" : "#2D2D44"}`,
            background: val === u ? "rgba(200,241,53,0.12)" : "transparent",
            color: val === u ? "#C8F135" : "#6B7280", fontSize: 10, cursor: "pointer" }}>
          {u}
        </button>
      ))}
    </div>
        ...
        <div style={{ padding: 24 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <div style={{ gridColumn: "1 / -1" }}>
              <label style={{ display: "block", fontSize: 10, fontWeight: 600, color: "#4B5563", marginBottom: 4, textTransform: "uppercase" }}>Nama *</label>
    ...
    <div style={{ fontFamily: "'DM Sans', Arial, sans-serif" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
        <div>
          ...
          <p style={{ fontSize: 12, color: "#6B7280", margin: "4px 0 0" }}>Manajemen barang non-bahan baku per kategori</p>
        </div>
        {!showMutasi && (
          ...
          return (
            <button key={t.key} onClick={() => { setActiveTab(t.key); setShowMutasi(false); }}
              style={{ padding: "6px 14px", borderRadius: 6, border: "none", cursor: "pointer", fontSize: 12,
                fontWeight: isActive ? 700 : 400, background: isActive ? "#1E1E2E" : "transparent",
                color: isActive ? "#E2E8F0" : "#4B5563", transition: "all 0.15s" }}>
              {t.label}
              {count > 0 && <span style={{ marginLeft: 5, fontSize: 10, color: isActive ? "#C8F135" : "#374151", fontWeight: 700 }}>{count}</span>}
            </button>
          );
        })}
        <button onClick={() => setShowMutasi(true)}
          style={{ padding: "6px 14px", borderRadius: 6, border: "none", cursor: "pointer", fontSize: 12,
            fontWeight: showMutasi ? 700 : 400, background: showMutasi ? "#1E1E2E" : "transparent",
            color: showMutasi ? "#E2E8F0" : "#4B5563" }}>
          Mutasi Antar Cabang
        </button>
      </div>
      ...
      {!showMutasi && (
        <div style={{ background: `var(--color-os-card)`, border: "1px solid var(--color-os-border)", borderRadius: 12, overflow: "hidden" }}>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            ...
            <tbody>
              {filtered.length === 0 ? (
                <tr><td colSpan={7} style={{ padding: 32, textAlign: "center", color: "#4B5563", fontSize: 12 }}>
      ...
      {editTarget && (
        <ModalForm
          title={<>Edit — <span style={{ color: "#60A5FA" }}>{editTarget.id}</span></>}
